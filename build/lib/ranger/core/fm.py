...
from ranger.ext import get_executables, logutils
...
class FM(Actions, SignalDispatcher):  # pylint: disable=too-many-instance-attributes
    ...
    def initialize(self):  # pylint: disable=too-many-statements
        ...
        def set_image_displayer():
            ...
            self.image_displayer = get_image_displayer(
                self.settings.preview_images_method
            )
        ...
        def sxiv_workaround_hook(command):
            ...
            if (
                ...
                and re.match(r"^(feh|n?sxiv|imv|pqiv) ", command)
            ):
                ...
                if images and self.thisfile.relative_path in images and "$@" in command:
                    ...
                    if command[0:5] == "pqiv ":
                        ...
                        new_command = command.replace(
                            "pqiv ",
                            'pqiv --action "goto_file_byindex(%d)" ' % number,
                            1,
                        )
        ...
        self.settings.signal_bind(
            ...
            lambda signal: signal.fm.bookmarks.enable_saving_backtick_bookmark(
                signal.value
            ),
        )
    ...
    def get_filesystem_objects(self, paths):
        ...
        for dirname, basenames in groups.items():
            ...
            if basenames != set():
                ...
                self.fm.notify(
                    "Error: No such file or directory: {0}".format(names), bad=True
                )
                return None
        return result
